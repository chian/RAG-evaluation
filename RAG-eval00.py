...
"""
#Example Test
answer_key = "2-(4-hydroxyphenyl)ethyl"
pattern = re.compile(r'\b' + re.escape(answer_key) + r'\b')
index = 1
for doc in docs:
    if pattern.search(doc.page_content):
        print("Found on index:",index)
...
with open(test_filename, 'r', encoding='utf-8') as file:
    for line in file:
        ...
        if name:
            query = "What is the compound ID for the compound named" + name.group() + "?"
            answer_key = pattern2.search(line)
            #print("#", query, answer_key.group())
            #answer_key = name.group()
            if answer_key:
                docs = retriever.get_relevant_documents(query)
                pattern_temp = re.compile(r'(?:^|\W)' + re.escape(str(answer_key.group())) + r'(?:$|\W)')
                #print(re.escape(str(answer_key.group())))
                index = 1
                index_list = []
                for doc in docs:
                    ...
                    if pattern_temp.search(str(doc.page_content).strip()):
                        ...
                        #print("##",doc.page_content)
                    index += 1
                print(name.group(), answer_key.group(), index_list)
                #print("#",docs[0].page_content)
                if len(index_list) == 0:
                    super_index_list.append(zone)

print("Average Search Rank:",sum(super_index_list)/len(super_index_list))
print("Average First Hit Rank:",sum(first_index_list)/len(first_index_list))
