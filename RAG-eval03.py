...
def VectorTest(test_filename, retriever, zone):
    super_index_list = []
    first_index_list = []
...
if tokenizer.pad_token is None:
    # Set to the default pad token for the tokenizer or a known pad token like '[PAD]'
    # For BERT-like models, '[PAD]' is commonly used, but you should adjust this based on the model
...
if (os.path.exists(args.persist_dir) and os.listdir(args.persist_dir)) and args.revector==False:
    ...
    loader = DirectoryLoader(args.input_dir, glob="testing_data.txt", loader_cls=TextLoader)
...
zone = len(vectordb.get()['documents']) #too large for medium sized datasets
#pdb.set_trace()
...
test_results = VectorTest(test_filename=test_filename, retriever=retriever, zone=zone)
#print(test_results)
...
adapter_filename = os.path.join(args.input_dir,"TMP_adapter")

if (os.path.exists(adapter_filename) and os.listdir(adapter_filename)) and args.revector==False:
    ...
    filename = os.path.join(args.input_dir, "training_data.txt")
    print(filename)
    ...
    peft_model = load_peft_adapter(args.model_name,adapter_filename)
...
if (os.path.exists(second_chroma_dir) and os.listdir(second_chroma_dir)) and args.revector==False:
    print("Loading existing ChromaDB from", second_chroma_dir,flush=True)
    vectordb = Chroma(embedding_function=embedding_function,persist_directory=second_chroma_dir)
else:
    print("Creating new ChromaDB at", second_chroma_dir,flush=True)
    # Load input files from input_dir
    loader = DirectoryLoader(args.input_dir, glob="testing_data.txt", loader_cls=TextLoader)
    documents = loader.load()
    text_splitter = RecursiveCharacterTextSplitter(chunk_size=256, chunk_overlap=100)
